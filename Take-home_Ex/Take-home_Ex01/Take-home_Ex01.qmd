...
```{r}
funckde_auto<- density(funcppp1.km,
                  sigma=bw.diggle,
                  edge=TRUE,
                  kernel="gaussian")

nonfunckde_auto<- density(nonfuncppp1.km,
                  sigma=bw.diggle,
                  edge=TRUE,
                  kernel="gaussian")
par(mfrow=c(1,2))
plot(funckde_auto, main="Functional Waterpoint KDE using Automatic Bandwidth")
plot(nonfunckde_auto, main="Non-Functional Waterpoint KDE using Automatic Bandwidth")
```
...
```{r}
gridded_kde_funckde <- as.SpatialGridDataFrame.im(funckde_auto)

gridded_kde_nonfunckde <- as.SpatialGridDataFrame.im(nonfunckde_auto)
```
...
```{r}
K_Fun <- Kest(funcppp, correction = "Ripley")

K_NonFun <- Kest(nonfuncppp, correction = "Ripley")
...
```{r}
K_Fun.csr <- envelope(funcppp, Kest, nsim = 39, rank = 1, glocal = TRUE)

K_NonFun.csr <- envelope(nonfuncppp, Kest, nsim = 39, rank = 1, glocal = TRUE)
```
...
To reinforce our analysis, we can use the second order spatial point pattern analysis to conclude the nature of dependence between functional and non-functional waterpoints--whether they are evenly distributed or clustered if the distribution of points is not random. Our hypothesis are stated below
...
### Using K function to check for distribution trend

We use the K function to analyse the trend of distribution, as done so in the earlier section.

```{r}
K_colocation <- Kest(funcppp, correction = "Ripley")
plot(K_colocation, xlim=c(0,500))
```

### Conduct Monte Carlo simulations for the K estimates

```{r}
K_CK.csr <- envelope(LCLQ_WP_ppp, Kest, nsim = 39)
```
...
From the graph, we see that there is evidence of clustering of the non-functional waterpoints about the functional waterpoints.
