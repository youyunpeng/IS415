...
length(which(st_is_valid(geoJAR) == FALSE)) geoJAR\[rowSums(is.na(geoJAR))!=0,\]
...
We also included a shiny app where the user can input the date desired, and the output would be the tmap graph corresponding to the date selected.
...
1.  Greater number of oscillating hot spots than oscillating cold spots
...
2.  Presence of Sporadic cold spots

This could imply the presence of areas in which vaccination rate often switch between insignificant levels of clustering and significant levels of clustering of low vaccination rates. 
